...
class Chatbot:
    def __init__(self):
        ...
        self.kakao1_path = os.path.join(self.data_path, 'KAKAO_1')

        self.kakao_path = []

        for folder in os.listdir(self.data_path):
            if "KAKAO" in folder:
                self.kakao_path.append(os.path.join(self.data_path, folder))
        
    def step1(self):
        for folder in self.kakao_path:
            if not os.path.exists(folder):
                print("카카오톡 대화 데이터셋이 존재하지 않습니다. AI hub에서 데이터셋을 다운로드 받으십시오.")
                return
    ...
    def deleteUselessFiles(self):
        for folder in self.kakao_path:
            if os.path.exists(os.path.join(folder, "KAKAO_1077_17.json")):
                os.remove(os.path.join(folder, "KAKAO_1077_17.json"))
                print("필요없는 데이터셋 삭제 완료")
            if os.path.exists(os.path.join(folder, "KAKAO_2876_15.json")):
                os.remove(os.path.join(folder, "KAKAO_2876_15.json"))
                print("필요없는 데이터셋 삭제 완료")
            if os.path.exists(os.path.join(folder, "KAKAO_3282_12.json")):
                os.remove(os.path.join(folder, "KAKAO_3282_12.json"))
                print("필요없는 데이터셋 삭제 완료")
            if os.path.exists(os.path.join(folder, "KAKAO_3321_01.json")):
                os.remove(os.path.join(folder, "KAKAO_3321_01.json"))
                print("필요없는 데이터셋 삭제 완료")
            if os.path.exists(os.path.join(folder, "KAKAO_3394_12.json")):
                os.remove(os.path.join(folder, "KAKAO_3394_12.json"))
                print("필요없는 데이터셋 삭제 완료")
            if os.path.exists(os.path.join(folder, "KAKAO_3399_03.json")):
                os.remove(os.path.join(folder, "KAKAO_3399_03.json"))
                print("필요없는 데이터셋 삭제 완료")
            if os.path.exists(os.path.join(folder, "KAKAO_4066_16.json")):
                os.remove(os.path.join(folder, "KAKAO_4066_16.json"))
                print("필요없는 데이터셋 삭제 완료")
            if os.path.exists(os.path.join(folder, "KAKAO_4226_16.json")):
                os.remove(os.path.join(folder, "KAKAO_4226_16.json"))
                print("필요없는 데이터셋 삭제 완료")
            if os.path.exists(os.path.join(folder, "KAKAO_4325_16.json")):
                os.remove(os.path.join(folder, "KAKAO_4325_16.json"))
                print("필요없는 데이터셋 삭제 완료")
            if os.path.exists(os.path.join(folder, "KAKAO_4404_17.json")):
                os.remove(os.path.join(folder, "KAKAO_4404_17.json"))
                print("필요없는 데이터셋 삭제 완료")
    ...
    def save_csv(self, result_df):
        result_df.to_csv('KakaoData.csv', index=False)
        print("데이터셋 저장 완료")
