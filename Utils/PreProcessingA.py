...
class Chatbot:
    def __init__(self):
        ...
        self.exceptKakao = os.path.join(self.data_path, 'NATEON')

        
    def step1(self):
            if not os.path.exists(self.exceptKakao):
                print("대화 데이터셋이 존재하지 않습니다. AI hub에서 데이터셋을 다운로드 받으십시오.")
                return
            
            else:
                print("데이터셋 파일 확인 완료")
                print("위치 : {0}".format(self.exceptKakao))
    ...
    def textInfo(self):
        chatdata = []
        json_file_list = os.listdir(self.exceptKakao)
        for i in range(len(json_file_list)):
            kakao_path_data_path = os.path.join(self.exceptKakao, json_file_list[i])
    ...
    def save_csv(self, result_df):
        result_df.to_csv('nateon.csv', index=False)
        print("데이터셋 저장 완료")
